...
class _AttributeArgumentRef(_ArgumentRef):
    ...
    def _get(self, arg_map: Mapping[str, Any]) -> Any:
        source = self._source._get(arg_map)  # pylint: disable=protected-access
        return getattr(source, self._attribute_name)
...
def check_shapes(*specs: ArgumentSpec) -> Callable[[_C], _C]:
    ...
    The specs passed to this decorator are (name, spec) tuples, where:
        name is a specification of the target to check the shape of.
            ...
            You can also use list lookup syntax (`argument[7]`) to access single members of tuples
                or lists.
        spec is definition of the expected shape. spec a sequence of one of:
    ...
    not verified in compiled mode. Examine the `statistics` global to monitor whether many of your
...
def _parse_argument_ref(argument_ref_str: str) -> _ArgumentRef:
    ...
    while start < len(argument_ref_str):
        ...
        if match:
            ...
            continue
        assert False, _assert_msg()
    assert start == len(argument_ref_str), _assert_msg()
    return result
...
class _InheritCheckShapes:
    ...
    def __set_name__(self, owner: type, name: str) -> None:
        ...
        setattr(self._func, "class_name", owner.__name__)
