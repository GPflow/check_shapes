...
class LinearModel(Model):
    ...
    def train(
        ...
        def loss_grads(weights: tjax.Array1[NFeatures]) -> tjax.Array1[NFeatures]:
            pred: tjax.Array2[NRows, N1] = self._predict(weights, training_features)
            grads: tjax.Array2[NRows, NFeatures] = (
                2 * (pred - training_targets) * training_features  # type: ignore
            )
            return cast(tjax.Array1[NFeatures], np.mean(grads, axis=0))
